...
class IncidentAnalysisEngine:
    ...
    def _generate_immediate_actions(self, incident: IncidentAnalysis) -> List[str]:
        ...
        Ne doporučení, ale CO DĚLAT TEĎ.
        """
        actions = []
        ...
        if incident.causal_chain:
            ...
            rc_app = incident.causal_chain.root_cause_app
            
            if 'database' in rc_type or 'pool' in rc_type:
                actions.append(f"Check DB connection pool on {rc_app}")
            elif 'timeout' in rc_type:
                actions.append(f"Check latency and dependencies of {rc_app}")
            elif 'memory' in rc_type or 'oom' in rc_type:
                actions.append(f"Check memory usage on {rc_app}, consider restart")
            elif 'external' in rc_type:
                actions.append(f"Verify external service availability")
            else:
                actions.append(f"Check logs and metrics on {rc_app}")
        
        # 2. Podle blast radius
        if incident.scope.blast_radius >= 3:
            downstream = incident.scope.apps[1] if len(incident.scope.apps) > 1 else "downstream services"
            actions.append(f"Verify {downstream} latency")
        
        # 3. Podle knowledge status
        if incident.knowledge_status == "NEW":
            actions.append("Prepare Jira ticket if persists >15 min")
    ...
    def _build_scope(self, events: List[Dict]) -> IncidentScope:
        ...
        for event in events:
            ...
            scope.total_errors += event.get('error_count', 0)
