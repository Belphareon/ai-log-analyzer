...
class FixRecommender:
    ...
    def _get_scope_based_actions(
        ...
        # Velký blast radius - BEZPEČNÁ diagnostika, ne rollback!
        if scope.blast_radius >= 5:
            actions.append(RecommendedAction(
                title="Assess cross-service impact",
                description=f"Incident affects {scope.blast_radius} applications - requires coordination",
                priority=ActionPriority.IMMEDIATE,
                category="operations",
                steps=[
                    "Check error rate trend (increasing/stable/decreasing)",
                    "Identify common dependency or recent deployment",
                    "Coordinate with affected teams",
                    "ESCALATE IF: error rate increasing >15 min → prepare rollback plan",
                ],
            ...
            if len(versions) >= 2:
                actions.append(RecommendedAction(
                    ...
                    steps=[
                        ...
                        "IF version-specific: consider targeted rollback",
                    ],
                    estimated_effort="30 min",
                    confidence=ConfidenceLevel.MEDIUM,
                ))
