...
def calculate_priority(
    ...
    propagation_time_sec: Optional[int] = None,
) -> Tuple['IncidentPriority', List[str]]:
    """
    Vypočítá prioritu incidentu.
    
    Pravidla (v5.3):
    - P1: NEW AND (CRITICAL OR blast_radius ≥3 OR namespaces ≥8 OR fast_propagation)
    - P2: KNOWN AND worsening
    - P2: NEW AND not critical
    - P3: KNOWN AND stable
    - P4: default
    
    v5.3: Rychlá propagace (< 30s přes 3+ apps) je P1 eskalace.
    ...
    # P1: NEW AND (CRITICAL OR major cross-app OR massive cross-ns OR fast propagation)
    if knowledge_status == "NEW":
        ...
        if blast_radius >= 3:
            ...
            return IncidentPriority.P1, reasons
        # Massive cross-namespace (8+ je neobvyklé)
        if namespace_count >= 8:
            reasons.append("new_incident")
            reasons.append("massive_cross_namespace")
            return IncidentPriority.P1, reasons
        # v5.3: Rychlá propagace = vážné
        if propagated and propagation_time_sec is not None and propagation_time_sec < 30 and blast_radius >= 2:
            reasons.append("new_incident")
            reasons.append("fast_propagation")
            return IncidentPriority.P1, reasons
    ...
    if knowledge_status == "NEW":
        reasons.append("new_incident")
        # v5.3: Pomalá propagace je méně urgentní
        if propagated and propagation_time_sec and propagation_time_sec > 300:
            reasons.append("slow_propagation")
        return IncidentPriority.P2, reasons
...
class IncidentScope:
    ...
    # === ROLE APPS (v5.3) ===
    root_apps: List[str] = field(default_factory=list)         # Příčina
    downstream_apps: List[str] = field(default_factory=list)   # Ovlivněné root
    collateral_apps: List[str] = field(default_factory=list)   # Vedlejší poškození
    ...
    version_change_app: Optional[str] = None
    version_change_from: Optional[str] = None
    version_change_to: Optional[str] = None
    ...
    def is_localized(self) -> bool:
        """Incident zůstal lokální (1 app)?"""
        return len(self.apps) <= 1
    
    @property
    def has_clear_root(self) -> bool:
        """Máme jasný root cause app?"""
        return len(self.root_apps) == 1
    
    def detect_version_changes(self):
        """
        Detekuje změny verzí v app_versions.
        
        OPRAVA v5.3: Semver-aware sorting místo string sorting.
        (String sort: "1.10.0" < "1.9.0" - špatně!)
        """
        for app, versions in self.app_versions.items():
            if len(versions) >= 2:
                # Semver-aware sort
                sorted_versions = self._sort_versions(list(versions))
                self.version_change_detected = True
                self.version_change_app = app
                self.version_change_from = sorted_versions[0]
                self.version_change_to = sorted_versions[-1]
                break
    
    def _sort_versions(self, versions: List[str]) -> List[str]:
        """
        Seřadí verze semver-aware.
        ...
        def version_key(v: str) -> tuple:
            ...
            # Pokusit se parsovat jako semver
            import re
            ...
            if match:
                ...
                prerelease = match.group(4) or 'zzzz'  # stable > prerelease
                return (major, minor, patch, prerelease)
            
            # Fallback - release-YYYY.MM format
            match = re.match(r'^release-(\d{4})\.(\d{2})(?:\.(\d+))?$', v)
            if match:
                return (int(match.group(1)), int(match.group(2)), 
                        int(match.group(3)) if match.group(3) else 0, 'zzzz')
            
            # Ultimate fallback - string sort
            return (0, 0, 0, v)
    ...
    def classify_app_roles(self, first_app: str, app_timestamps: Dict[str, 'datetime']) -> 'IncidentPropagation':
        ...
        sorted_apps = sorted(
            [(app, ts) for app, ts in app_timestamps.items() if app != first_app],
            key=lambda x: x[1]
        )
        
        if sorted_apps and first_app and first_app in app_timestamps:
            first_ts = app_timestamps[first_app]
            
            # Downstream = apps do 60s od root
            # Collateral = apps po 60s
            for app, ts in sorted_apps:
                delta = (ts - first_ts).total_seconds()
                if delta <= 60:
                    self.downstream_apps.append(app)
                else:
                    self.collateral_apps.append(app)
