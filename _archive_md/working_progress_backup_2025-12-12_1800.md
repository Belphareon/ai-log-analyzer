...
**Poslední aktualizace:** 2025-12-12 17:10 UTC  
**Status:** Phase 4 COMPLETE ✅ - Peak Detection Data Collection IN PROGRESS

---

## 🔍 PEAK DETECTION DATA COLLECTION - KNOWLEDGE BASE

### Databáze - Uživatelé a Hesla
```
Host: P050TD01.DEV.KB.CZ:5432/ailog_analyzer
Schéma: ailog_peak

DDL User (pro CREATE SCHEMA/TABLE):
  User: ailog_analyzer_ddl_user_d1
  Pass: WWvkHhyjje8YSgvU
  Role: role_ailog_analyzer_ddl

Data User (pro INSERT/SELECT):
  User: ailog_analyzer_user_d1
  Pass: y01d40Mmdys/lbDE
  Role: role_ailog_analyzer
```
...
⚠️ POZNÁMKA: Agregace v ES má problém s vrácením všech namespace
Řešení: Používat analyze_period.py (orchestrace) - ta vrací správně všechny aplikace

Data za poslední 24h (2025-12-11 16:00 - 2025-12-12 16:00):
  - Celkem errorů: 59,031
  - Aplikace: 44 (včetně bff-pcb-ch-*)
  - Clustery: 2 (cluster-k8s_nprod_3095-in, cluster-k8s_nprod_3100-in)
```

### Skript Workflow - Správný Postup
```
1. OVĚŘENÍ DAT (bez DB):
   $ python3 fetch_unlimited.py --from "2025-12-11T16:00:00Z" --to "2025-12-12T16:00:00Z" --output /tmp/errors.json
   Výstup: JSON s 59,031 errory, všemi aplikacemi a namespace

2. ANALÝZA (orchestrace):
   $ python3 analyze_period.py --from 2025-12-11T16:00:00Z --to 2025-12-12T16:00:00Z --output /tmp/analysis
   Výstup: JSON s detailní analýzou, trace extrakací, root causes

3. PEAK DETECTION (do DB):
   $ python3 collect_historical_peak_data.py --days 1
   Konfigurace: Musí používat track_total_hits: True v ES query
   Insert: Do tabulky ailog_peak.peak_statistics (s SET ROLE DDL)
```

### Problémy a Řešení
```
PROBLÉM 1: collect_historical_peak_data.py vrací jen 385 kombinací místo všech
PŘÍČINA: Agregace v ES má limity, nevrací všechny namespace
ŘEŠENÍ: ✅ OPRAVENO - Přidán track_total_hits: True
OVĚŘIT: Během sběru za 24h - měla by vrátit více kombinací

PROBLÉM 2: Některé namespace chybí (pca-fat, pca-uat, pcb-ch-*)
PŘÍČINA: Agregace limituje výstup na top-N namespace
POZNÁMKA: Data tam jsou - vidíme je v analyze_period.py (bff-pcb-ch-*)
ŘEŠENÍ: ✅ PENDING - Potřeba ověřit výstup collect scriptu

PROBLÉM 3: DB insert bez ověření dat
ŘEŠENÍ: ✅ TODO - Nejdřív ověřit JSON výstup, pak insert
```

### Setup Tabulky - HOTOVO ✅
```
Script: setup_peak_db.py
Login: ailog_analyzer_ddl_user_d1 / WWvkHhyjje8YSgvU
Výstup:
  ✅ Schema ailog_peak created
  ✅ Table peak_statistics created (2623 rows z testů)
  ✅ Permissions granted to ailog_analyzer_user_d1

Tabulka peak_statistics struktura:
  - day_of_week (0-6: Mon-Sun)
  - hour_of_day (0-23)
  - quarter_hour (0-3: 15min intervaly)
  - namespace (VARCHAR 255)
  - mean_errors, stddev_errors
  - samples_count, last_updated
```

---

## 📊 SESSION - 2025-12-12 17:10 UTC - Peak Data Collection Investigation

### 🎯 Cíl
- ✅ Pochopit správný workflow pro sběr peak detection dat
- ✅ Ověřit které scripty fungují správně
- 🔄 Připravit data za poslední 24h bez bordelu v DB
- 📋 Dokumentovat veškeré poznatky

### ✅ Co se zjistilo

**Správný sběr dat (OVĚŘENO):**
1. `fetch_unlimited.py` - Vrací ALL 59,031 errorů za 24h ✅
2. `analyze_period.py` - Orchestrace s správnými aplikacemi/namespace ✅
3. Všechny aplikace včetně bff-pcb-ch-* tam jsou ✅

**Problém s agregacemi (IDENTIFIED):**
- `collect_historical_peak_data.py` agregace má limity na namespace
- Vrací jen 385 kombinací (malý subset)
- ✅ FIXED: Přidán `track_total_hits: True` do ES query

**Uživatelé a práva (OVĚŘENO):**
- DDL user `ailog_analyzer_ddl_user_d1` - funguje pro CREATE ✅
- Data user `ailog_analyzer_user_d1` - funguje pro INSERT (po GRANT) ✅
- Setup script vytváří schema a tabulky bez problémů ✅

### 🔄 CO JE POTŘEBA UDĚLAT

**KROK 1: Ověřit data za 24h (BEFORE DB INSERT)**
```
Spustit: python3 collect_historical_peak_data.py --days 1
Ověřit: 
  - Počet stažených kombinací (mělo by být > 385, ideálně 500+)
  - Všechny namespace v datech (včetně pcb-ch)
  - Správné mean/stddev hodnoty s smoothing
Výstup: JSON soubor (ne rovně do DB!)
```

**KROK 2: Kontrola uživatelem**
- Zkontrolovat že jsou všechna data správně
- Že počty errorů dávají smysl
- Že jsou tam všechny namespace

**KROK 3: Vyčistit DB a udělat to pořádně**
```
DELETE FROM ailog_peak.peak_statistics WHERE last_updated > '2025-12-12 10:00:00';
```

**KROK 4: Spustit sběr pro všechny dostupné daty (21+ dní)**
```
python3 collect_historical_peak_data.py --days 21
```

---

## 📊 SESSION - 2025-12-12 15:50 UTC - Context Protocol & Progress Update



### 🎯 Cíl Session
- ✅ Načíst CONTEXT_RETRIEVAL_PROTOCOL.md a získat kontext projektu
- ✅ Aktualizovat working_progress.md s dnešním timestampem
- 🔄 Připravit strukturu pro práci po menších krocích

### Status Updates (Real-time)

**15:45 UTC - Session Start**
- ✅ Načten CONTEXT_RETRIEVAL_PROTOCOL.md (kompletní kontext)
- ✅ Načten working_progress.md (poslední update 2025-12-06)
- ✅ Zkontrolován aktuální stav: Phase 4 Complete
- 📝 Vytvořen nový entry v progress log

**15:50 UTC - Context Verified**
- ✅ Project Status: Phase 4 COMPLETE ✅
- ✅ Docker Image: v0.4.0-docker-verified + latest (Harbor)
- ✅ K8s Manifests: Ready in feature/ai-log-analyzer-v2
- ✅ Database: 2608 baseline records (known errors)
- ✅ Security: Soteri scan passed

**16:00 UTC - BLOCKER IDENTIFIED (3 days ago)**

🚨 **Problem 1: ImagePullBackOff - Unauthorized Harbor Access**
```
Error: unauthorized to access repository: pccm-sq016/ai-log-analyzer
```

**Root Cause:**
- `values.yaml` references: `harbor-docker-registry-secret` 
- Template creates: `ai-log-analyzer-docker-registry-secret`
- ❌ **Name mismatch!** Pod can't find the secret

🚨 **Problem 2: Empty Conjur Secrets (0 bytes)**
```
ai-log-analyzer-secret:
  dbUsername: 0 bytes
  dbPassword: 0 bytes
  esUsername: 0 bytes
  esPassword: 0 bytes
...
**Root Cause:**
- `_conjur.tpl` hardcodes: `ai-log-analyzer-secret`
- Actual secret name: `{{ include "ai-log-analyzer.fullname" . }}-secret`
- ❌ **Conjur init container updating wrong secret!**

**16:15 UTC - FIXES APPLIED ✅**

1. **Fixed imagePullSecret name** (`values.yaml`)
   ```diff
   - name: harbor-docker-registry-secret
   + name: ai-log-analyzer-docker-registry-secret
   ```

2. **Fixed Conjur K8S_SECRETS** (`_conjur.tpl`)
   ```diff
   - value: "{{ .Release.Namespace }}/ai-log-analyzer-secret"
   + value: "{{ .Release.Namespace }}/{{ include \"ai-log-analyzer.fullname\" . }}-secret"
   ```

**16:20 UTC - Changes Committed & Pushed ✅**
- Commit: `18c0969` - "Fix: Conjur secret name and imagePullSecret mismatch"
- Branch: `feature/ai-log-analyzer-v2`
- Status: Pushed to remote, waiting for ArgoCD sync

**16:30 UTC - NEW BLOCKER IDENTIFIED: Conjur 401 Unauthorized ❌**

🚨 **Problem 3: Conjur Authentication Failure**
```
ERROR: CAKC029 Received invalid response to certificate signing request. Reason: status code 401
ERROR: CAKC015 Login failed
ERROR: CSPFK010E Failed to authenticate for AI_LOG_ANALYZER authenticator
```

**Root Cause Analysis:**
- Pod tries to authenticate as: `host/conjur/authn-k8s/nprod/AP_CLD_08901_SQ016/AI_LOG_ANALYZER/ai-log-analyzer/k8s-provider`
- Conjur returns: **401 Unauthorized**
- **Důvod:** Host identity neexistuje v Conjur policy NEBO nemá permissions

**Failed Secret Paths:**
- `epv/CAR_TA_LOBUser_TEST/DAN_PCB/DAN_PCB-jdbcAilogUserDual/username`
- `epv/CAR_TA_LOBUser_TEST/DAN_PCB/DAN_PCB-jdbcAilogUserDual/password`
- `epv/CAR_TA_LOBUser_TEST/DAP_PCB/XX_PCBS_ES_READ/username`
- `epv/CAR_TA_LOBUser_TEST/DAP_PCB/XX_PCBS_ES_READ/password`

**Status:** ⏳ **BLOCKED** - Vyžaduje akci Cyberark/DevOps týmu
**Known Since:** 2025-12-09 (viz Conjur_Integration_Progress.md)

**Required Action:** Create Conjur host identity + assign permissions

**16:30 UTC - NEW BLOCKER: Conjur Authentication 401 🚨**

ArgoCD synced, ale Conjur init container selhává s 401:
```
ERROR: CAKC029 Received invalid response to certificate signing request. Reason: status code 401
ERROR: CSPFK010E Failed to authenticate for AI_LOG_ANALYZER authenticator
```
...
**Authenticating as:**
```
host/conjur/authn-k8s/nprod/AP_CLD_08901_SQ016/AI_LOG_ANALYZER/ai-log-analyzer/k8s-provider
```

**Root Cause Analysis:**
1. ✅ ImagePullSecret fixed - pod nyní stahuje image
2. ✅ Secret name fixed - Conjur init container se spouští
3. ❌ **Conjur host identity neexistuje nebo nemá permissions**

**From Conjur_Integration_Progress.md (9.12.2025):**
- Stejný problém již řešen před 3 dny
- Host identity path je správně sestavený (podle pattern ostatních komponent)
- **Problém:** Identity `AI_LOG_ANALYZER/ai-log-analyzer` pravděpodobně není vytvořená v Conjuru
- **Nebo:** Nemá permissions na secret paths v `epv/CAR_TA_LOBUser_TEST/...`
...
**Next Actions:**
- [ ] Kontaktovat Cyberark/DevOps tým pro vytvoření host identity
- [ ] Nebo ověřit v Conjur UI, jestli identity existuje
- [ ] Zkontrolovat permissions na secret paths

### 📋 Co je HOTOVO (z context)
1. **Docker Image** - Buildnutý a pushnutý do Harbor
   - Tag: `v0.4.0-docker-verified` + `latest`
   - Registry: `dockerhub.kb.cz/pccm-sq016/ai-log-analyzer`

2. **K8s Manifests** - Validované YAML
   - Location: `/home/jvsete/git/sas/k8s-infra-apps-nprod/infra-apps/ai-log-analyzer/`
   - Branch: `feature/ai-log-analyzer-v2`

3. **Database** - Baseline data
   - 2608 záznamů known errors v PostgreSQL
   - Host: P050TD01.DEV.KB.CZ

4. **Security** - Soteri compliant
   - No PASSWORD_IN_URL ✅

### 🔄 CO ZBÝVÁ (Phase 5)
- [ ] Deploy to K8s cluster nprod-3100
- [ ] Verify DNS: ai-log-analyzer.sas.kbcloud
- [ ] Verify database dual account setup
- [ ] Integration testing
- [ ] Monitor pod startup
- [ ] Test health endpoint: `/api/v1/health`

### 📝 Notes
- CONTEXT_RETRIEVAL_PROTOCOL.md existuje a obsahuje kompletní přehled
- Workflow pro pokračování v práci dokumentován
- Známé problémy + řešení zdokumentovány
- Reference na všechny důležité dokumenty připraveny

**Next Step:** Pokračovat s Phase 5 deployment verification (postupovat po malých krocích)
