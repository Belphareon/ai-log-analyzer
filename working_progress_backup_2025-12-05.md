...
## 🔄 CLUSTER DEPLOYMENT DECISION - 2025-12-05

**Decision:** Move from local development to Kubernetes cluster deployment

**Reason:** Continuous operation requires:
- ✅ Automated scheduling (CronJob every 15 min)
- ✅ Real production data (from actual ES cluster)
- ✅ DB persistence (P050TD01 accessible from cluster)
- ✅ Multi-environment support (nprod, prod)
- ✅ Proper logging and monitoring

**Impact on Implementation:**

1. **Phase 2: collect_peak_data_continuous.py**
   - Deploy as Kubernetes CronJob (every 15 minutes)
   - Image: Custom container with Python + psycopg2 + ES client
   - ConfigMap: DB credentials, ES endpoints
   - ServiceAccount: Cluster access if needed

2. **Phase 3: analyze_period.py modification**
   - Deploy as existing orchestration (already running on cluster)
   - Modify to query peak_statistics from DB
   - Add peak detection logic

3. **Infrastructure needed:**
   - Dockerfile: Build container image
   - k8s/cronjob.yaml: CronJob manifest
   - k8s/configmap.yaml: Configuration (endpoints, credentials)
   - Helm chart (optional): For easy deployment across envs

**Next Steps:**
1. Create Dockerfile for peak detection service
2. Create Kubernetes manifests (CronJob, ConfigMap, ServiceAccount)
3. Deploy to NPROD cluster first
4. Test with real ES data
5. Scale to other environments

**Status:** ✅ DECISION MADE | 🔄 TODO & PROGRESS UPDATED
