#!/usr/bin/env python3
"""
Test script to verify ES 10K window limit - with retry logic
"""
...
def test_limit(from_idx, size, label, retry_count=3):
    """Test specific from/size combination with retry"""
    for attempt in range(retry_count):
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                print(f"   ⚠️  Auth issue (attempt {attempt+1}/{retry_count})")
                if attempt < retry_count - 1:
                    time.sleep(2)
                    continue
                else:
                    error = resp.json()
                    print(f"   ❌ FAILED after retries: {error.get('error', {}).get('type', 'Auth error')}")
                    return False
            else:
                error = resp.json()
...
print("=" * 70)
print(f"Target: {INDICES}")
print(f"Time range: 2025-12-02T07:30:00Z to 2025-12-02T10:30:00Z")
print(f"Level: ERROR")
