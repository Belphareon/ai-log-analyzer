...
def guess_category(error_type: str, message: str) -> IncidentCategory:
    ...
    VYLEPŠENO v5.2: Více fallback pravidel pro méně "unknown".
    """
    ...
    if any(kw in text for kw in ['deadlock', 'lock wait timeout', 'could not serialize']):
        return IncidentCategory.DATABASE
    if any(kw in text for kw in ['database', 'sql', 'jdbc', 'postgres', 'mysql', 'oracle', 'constraint violation']):
        return IncidentCategory.DATABASE
    # Exception classes
    if any(kw in error_type_lower for kw in ['sqlexception', 'dataaccess', 'hibernate', 'jpa', 'transient']):
        return IncidentCategory.DATABASE
    
    # === TIMEOUT ===
    if any(kw in text for kw in ['timeout', 'timed out', 'read timed', 'connect timed', 'sockettimeout']):
        return IncidentCategory.TIMEOUT
    if 'timeoutexception' in error_type_lower:
        return IncidentCategory.TIMEOUT
    
    # === NETWORK ===
    if any(kw in text for kw in ['connection refused', 'connection reset', 'econnrefused', 'econnreset']):
        return IncidentCategory.NETWORK
    if any(kw in text for kw in ['network', 'dns', 'broken pipe', 'socket', 'connect exception']):
        return IncidentCategory.NETWORK
    if any(kw in text for kw in ['unreachable', 'host not found', 'no route']):
        return IncidentCategory.NETWORK
    # Exception classes
    if any(kw in error_type_lower for kw in ['ioexception', 'socketexception', 'connectexception']):
        return IncidentCategory.NETWORK
    
    # === AUTH ===
    if any(kw in text for kw in ['401', '403', 'unauthorized', 'forbidden', 'auth', 'token expired']):
        return IncidentCategory.AUTH
    if any(kw in text for kw in ['access denied', 'permission denied', 'credentials', 'authentication']):
        return IncidentCategory.AUTH
    if any(kw in error_type_lower for kw in ['security', 'access', 'authent', 'authoriz']):
        return IncidentCategory.AUTH
    
    # === BUSINESS ===
    if any(kw in text for kw in ['not found', 'does not exist', '404', 'validation', 'invalid']):
        return IncidentCategory.BUSINESS
    if any(kw in text for kw in ['illegalargument', 'constraint', 'missing', 'required']):
        return IncidentCategory.BUSINESS
    if any(kw in text for kw in ['nullpointer', 'null value', 'cannot be null']):
        return IncidentCategory.BUSINESS
    # Exception classes
    if any(kw in error_type_lower for kw in ['illegalargument', 'illegalstate', 'validation', 'nullpointer']):
        return IncidentCategory.BUSINESS
    
    # === MEMORY ===
    if any(kw in text for kw in ['outofmemory', 'oom', 'heap', 'gc overhead', 'memory']):
        return IncidentCategory.MEMORY
    if 'outofmemory' in error_type_lower:
        return IncidentCategory.MEMORY
    
    # === EXTERNAL ===
    if any(kw in text for kw in ['external', '503', '502', '500', 'upstream', 'service unavailable', '429', 'rate limit']):
        return IncidentCategory.EXTERNAL
    if any(kw in text for kw in ['downstream', 'dependency', 'circuit breaker', 'fallback']):
        return IncidentCategory.EXTERNAL
    # HTTP status codes (jako fallback)
    if any(kw in text for kw in ['bad gateway', 'gateway timeout', 'internal server error']):
        return IncidentCategory.EXTERNAL
    
    # === FALLBACK: Exception class heuristika ===
    if 'exception' in error_type_lower:
        # Runtime errors jsou často business/code issues
        if 'runtime' in error_type_lower:
            return IncidentCategory.BUSINESS
        # Checked exceptions jsou často externí
        if any(kw in error_type_lower for kw in ['io', 'remote', 'service']):
            return IncidentCategory.EXTERNAL
