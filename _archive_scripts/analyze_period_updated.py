...
Runs: Fetch → Extract → Report → Intelligent Analysis → Single Output File
(Updated 2025-12-08: Added STEP 5 - Detailed Intelligent Analysis)
"""
...
def analyze_period(date_from, date_to, output_file, batch_size=5000, enable_intelligent=True):
    ...
    # STEP 1: Fetch
    log_step("STEP 1/5: Fetching errors from Elasticsearch")
    ...
    # STEP 2: Extract
    log_step("STEP 2/5: Extracting root causes from traces")
    ...
    # STEP 3: Report
    log_step("STEP 3/5: Generating detailed analysis report")
    ...
    if enable_intelligent:
        log_step("STEP 4/5: Running Intelligent Analysis for deeper insights")
        intelligent_file = "/tmp/intelligent_analysis.json"
        if run_cmd(f"python3 intelligent_analysis.py --input {batch_file} --output {intelligent_file}", "Intelligent Analysis", show_progress=True):
    ...
    final_step = "STEP 5/5" if enable_intelligent else "STEP 4/4"
    log_step(f"{final_step}: Creating comprehensive analysis file")
    ...
    analysis_output = {
        "metadata": {
            "analysis_type": "Complete Trace-Based Root Cause Analysis with Intelligent Analysis",
        ...
        "intelligent_analysis": intelligent_analysis  # Can be None if disabled or failed
    }
    ...
    if enable_intelligent and intelligent_analysis:
        ...
        if ia_stats:
            print(f"  Trace-based patterns:        {ia_stats.get('total_traces', 0):>10,d}")
            print(f"  Error classes detected:      {ia_stats.get('error_classes', 0):>10,d}")
            print(f"  Temporal anomalies:          {ia_stats.get('temporal_anomalies', 0):>10,d}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Complete AI Log Analysis Pipeline with Intelligent Analysis')
    ...
    parser.add_argument('--no-intelligent', action='store_true', help='Disable intelligent analysis')

    args = parser.parse_args()
    enable_intelligent = not args.no_intelligent
    success = analyze_period(args.date_from, args.date_to, args.output, args.batch_size, enable_intelligent)
